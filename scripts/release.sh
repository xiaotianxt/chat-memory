...
REPO_SLUG="${REPO_SLUG:-xiaotianxt/chat-memory}"
TAP_NAME="${TAP_NAME:-xiaotianxt/tap}"
FORMULA_REF="${FORMULA_REF:-xiaotianxt/tap/chat-memory}"
...
Create a chat-memory release, wait for GitHub Actions to publish the arm64
artifact, update the Homebrew tap formula including its service block, and
verify brew.
...
Options:
  ...
  --skip-tests         Do not run make check before tagging.
...
Environment:
  REPO_SLUG            GitHub repo slug. Default: xiaotianxt/chat-memory
  TAP_NAME             Homebrew tap name. Default: xiaotianxt/tap
  FORMULA_REF          Brew formula ref. Default: xiaotianxt/tap/chat-memory
...
if [[ "$UPDATE_TAP" -eq 1 ]]; then
  ...
  FORMULA_PATH="${TAP_DIR}/Formula/chat-memory.rb"
...
ASSET_NAME="chat-memory-${TAG}-darwin-arm64.tar.gz"
...
FORMULA
  ...
  if [[ -z "$(git -C "$TAP_DIR" status --porcelain -- Formula/chat-memory.rb)" ]]; then
    log "tap already points to ${VERSION}"
  else
    git -C "$TAP_DIR" diff --check -- Formula/chat-memory.rb
    git -C "$TAP_DIR" add Formula/chat-memory.rb
    git -C "$TAP_DIR" commit -m "chat-memory ${VERSION}"
